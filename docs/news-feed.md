...
This feature turns external news APIs into a positivity-filtered feed that both the API and client can consume.

## How it works
- `scripts/news/generateFeed.js` fetches each comma-separated URL in `NEWS_API_ENDPOINTS` and normalizes common response shapes (`articles`, `data`, `results`, or a top-level array).
...
- The resulting articles are cached as JSON at `NEWS_OUTPUT_PATH` (defaults to `api/data/news-feed.json`). When no endpoints are configured, a small built-in fallback feed is used so the UI never goes empty.
...
- `NEWS_API_ENDPOINTS`: Comma-separated URLs to upstream news APIs.
- `NEWS_API_KEY`: Bearer token added to each upstream request (omit if not needed).
