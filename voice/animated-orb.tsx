...
export default function Orb({
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      const smoothing = target > currentHover ? 0.35 : 0.18;
      currentHover += (target - currentHover) * smoothing;
      if (Math.abs(target - currentHover) < 0.005) {
        currentHover = target;
      }
      ...
      if (rotateOnHover) {
        const activityFactor =
          typeof normalizedActivity === 'number' ? normalizedActivity : targetHover;
        const rotationFactor = 0.4 + activityFactor * 1.3;
  ...
  }, [hue, hoverIntensity, rotateOnHover, glow, isStatic, normalizedActivity]);
