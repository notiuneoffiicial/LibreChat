...
export default function Orb(props: OrbProps) {
  ...
  `
    ...
    }, [hue, hoverIntensity, rotateOnHover, forceHoverState, glow, isStatic])

    if (isStatic) {
        // Calculate hue-adjusted colors for static preview
        const hueRad = (hue * Math.PI) / 180
        const cos = Math.cos(hueRad)
        const sin = Math.sin(hueRad)

        // Base colors from shader
        const baseColor1 = [0.611765, 0.262745, 0.996078]
        const baseColor2 = [0.298039, 0.760784, 0.913725]

        // Simple hue adjustment
        function adjustHue(rgb: number[]) {
            const [r, g, b] = rgb
            // Convert to YIQ
        ...
        const color1Hex = `rgb(${Math.round(adjustedColor1[0] * 255)}, ${Math.round(adjustedColor1[1] * 255)}, ${Math.round(adjustedColor1[2] * 255)})`
        const color2Hex = `rgb(${Math.round(adjustedColor2[0] * 255)}, ${Math.round(adjustedColor2[1] * 255)}, ${Math.round(adjustedColor2[2] * 255)})`
        ...
        return (
            <div
                style={{
                    ...
                    position: "relative",
                    overflow: "hidden",
                    boxShadow: `0 0 ${50 * glow}px ${color2Hex}${Math.round(
                        64 * glow
                    )
                        .toString(16)
                        .padStart(2, "0")}`,
