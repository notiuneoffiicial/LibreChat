...
- **Status:** In progress (design & scoping)
- **Key Paths (repo):**
...
- **Commit Digest (summaries):** _None yet_
- **User Steps(things the user needs to do if any that codex does not have access to):** Upload constitution/global documents into `kb_global` via admin tooling once tables are provisioned.
